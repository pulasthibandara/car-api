...
trait OAuth2InfosTable extends HasDatabaseConfigProvider[PgSlickProfile] {
  import profile.api._

  protected class OAuth2Infos(tag: Tag) extends Table[DBOAuth2Info](tag, "oauth2_info") {
  ...
  val auth2Infos = TableQuery[OAuth2Infos]
}
...
class OAuth2InfoDAO @Inject() (
  ...
  protected def oAuth2InfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOAuth2Info <- auth2Infos if dbOAuth2Info.loginInfoId === dbLoginInfo.id
  } yield dbOAuth2Info
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- loginInfoQuery(loginInfo).joinLeft(auth2Infos).on(_.id === _.loginInfoId)
    } yield result
